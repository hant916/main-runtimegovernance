...
Clarify validates the governance thesis with real-world evidence ingestion. It is not
a peer core, not the owner of Ailuros strategy, and not a core dependency.

## Phase 1 — Evidence-Only Integration

Phase 1 of Clarify integration is **evidence-only**. No runtime control flows from the
browser into Ailuros, and no governance decisions flow back to Clarify for enforcement.

### What Phase 1 Includes

- **Evidence timeline ingestion:** Clarify sends structured evidence records (web
  navigation events, user interactions) that Ailuros stores in its run timeline.
...
| Browser adapter implementation | Excluded | Adapter contract exists but no browser adapter |
