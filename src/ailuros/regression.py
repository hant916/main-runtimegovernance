...
class GovernanceDimensionDelta(BaseModel):
    """The before/after values and deterministic transition for one dimension."""
...
class GovernanceRegressionDelta(BaseModel):
    """Read model for comparing two existing execution projections."""
...
def _budget_state(record: BudgetRecord) -> str:
    ...
    if (
        status in _BUDGET_EXCEEDED_STATUSES
        or (
