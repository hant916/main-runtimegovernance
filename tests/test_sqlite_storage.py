...
def test_storage_round_trips_records(tmp_path: Path) -> None:
...
def test_concurrent_event_append_same_run_produces_unique_sequences(tmp_path: Path) -> None:
    ...
    sequences = [r.sequence for r in results if r.sequence is not None]
...
def _make_run_for_storage(storage: SQLiteStorage, run_id: str) -> None:
    run = Run(
        run_id=run_id,
        agent_id="agent",
        environment=Environment.DEVELOPMENT,
...
class TestListRunsPagination:
    ...
    def test_limit_with_offset(self, tmp_path: Path) -> None:
        storage = SQLiteStorage(tmp_path / "test.sqlite")
        storage.init()
        for i in range(5):
            _make_run_for_storage(storage, f"run_{i}")
...
def test_corrupt_json_raises_explicit_error(tmp_path: Path) -> None:
    storage = SQLiteStorage(tmp_path / "runtime.sqlite")
    storage.init()
