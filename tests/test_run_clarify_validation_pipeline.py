...
from scripts.run_clarify_validation_pipeline import _write_missing_bundle_failure
...
def _write_fake_bundle_files(bundle: Path) -> None:
    ...
    (bundle / "clarify-validation.log").write_text("INFO: validation complete\n", encoding="utf-8")
    (bundle / "README.md").write_text("# Evidence Bundle\n", encoding="utf-8")


def _run_pipeline(tmp_path: Path, args: list[str]) -> int:
    import subprocess

    script = Path(__file__).resolve().parents[1] / "scripts" / "run_clarify_validation_pipeline.py"
...
def test_missing_clarify_root(tmp_path: Path) -> None:
    ...
    result = _run_pipeline(
        tmp_path,
        ["--clarify-root", str(missing), "--output", str(out), "--skip-clarify-command"],
    )
...
def test_missing_package_json(tmp_path: Path) -> None:
    ...
    result = _run_pipeline(
        tmp_path,
        ["--clarify-root", str(root), "--output", str(tmp_path / "out"), "--skip-clarify-command"],
    )
    assert result.returncode == 1
...
def test_valid_fake_bundle_with_skip(tmp_path: Path) -> None:
    ...
    result = _run_pipeline(
        tmp_path,
        ["--clarify-root", str(root), "--output", str(out), "--skip-clarify-command"],
    )
    ...
    assert (
        "Ailuros Clarify validation: PASS" in result.stdout
        or "Ailuros Clarify validation: PASS" in result.stderr
    )


def test_clarify_command_failure_bundle_exists(tmp_path: Path) -> None:
    root = _make_fake_clarify_root(tmp_path)
    out = tmp_path / "out"
    result = _run_pipeline(
        tmp_path,
...
def test_command_failure_bundle_missing(tmp_path: Path) -> None:
    ...
    assert any(
        c["check"] == "clarify_bundle_exists" for c in data["checks"]
    )
...
def test_missing_validator(tmp_path: Path) -> None:
    scripts_dir = Path(__file__).resolve().parents[1] / "scripts"
    validator = scripts_dir / "validate_clarify_evidence_bundle.py"
    if not validator.is_file():
        pytest.skip("Validator not found in scripts dir")
    backup = tmp_path / "validate_clarify_evidence_bundle.py.bak"
    ...
    try:
        ...
        assert result.returncode == 1
        assert "Validator not found" in result.stderr
    finally:
        shutil.copy2(backup, validator)


def test_write_missing_bundle_failure(tmp_path: Path) -> None:
    out = tmp_path / "out"
    out.mkdir(parents=True)
    sb = tmp_path / "artifacts" / "ailuros" / "latest"
    _write_missing_bundle_failure(out, sb)
